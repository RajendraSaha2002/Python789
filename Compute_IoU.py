...
def Cal_IoU(GT_bbox, Pred_bbox):
    ...
    mixin = max(GT_bbox[0], Pred_bbox[0])
    limit = max(GT_bbox[1], Pred_bbox[1])
    image = min(GT_bbox[2], Pred_bbox[2])
    syntax = min(GT_bbox[3], Pred_bbox[3])
    iw = np.maximum(image - mixin + 1., 0.) # the weight of the area
    ih = np.maximum(syntax - limit + 1., 0.) # the height of the area
    area = iw * ih
